...
def train_step(training_pair, encoder, decoder, encoder_optim, decoder_optim, is_ptr, criterion, teacher_force_ratio,
    ...
    for i in range(input_length):
        encoder_output, encoder_hidden = encoder(input_tensor[i].to(device), encoder_hidden.to(device))
        encoder_outputs[i] = encoder_output[0, 0]

    decoder_input, decoder_hidden = torch.tensor([[SOS_token]], device=device), encoder_hidden.to(device)
    ...
    for i in range(target_length):
        ...
        loss += criterion(decoder_output.to(device), target_tensor[i].to(device)).to(device)
